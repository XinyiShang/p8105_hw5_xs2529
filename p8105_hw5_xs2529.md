...
```
    ...
    ## # A tibble: 5,000 × 3
    ##    true_mu reject_null estimate_mu
    ##      <dbl>       <dbl>       <dbl>
    ##  1       0           0       0.412
    ##  2       0           0       0.664
    ##  3       0           0       0.551
...
hypothesis $H: \mu = 0$ as the true $\mu$ increases. Consequently, the
...
The average estimate of $\hat{\mu}$ tends to approach the true value of
$\mu$, as $\mu$ increases. This occurs because as the true $\mu$ moves
further from zero, the likelihood of rejecting the null hypothesis
increases. Consequently, the rejected group’s $\hat{\mu}$ values move
closer to the true $\mu$. Therefore, the average estimate $\hat{\mu}$
for the rejected cases should converge to the overall average estimate
of $\hat{\mu}$.
