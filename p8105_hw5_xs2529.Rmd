...
At $\mu = 0$, the average estimate of $\hat{\mu}$ is `r (sim_results_0 |> select(estimate_mu))[[1]] |> mean()`. Out of `r nrow(sim_results_0)` trials, `r sim_results_0 |> filter(reject_null == 1) |> nrow()` resulted in rejecting the null hypothesis $H: \mu = 0$. 
...
The power increases with the increase of the true $\mu$. This is attributed to the increase of the probability of rejecting the null hypothesis $H: \mu = 0$ as the true $\mu$ increases. Consequently, the power, which is the proportion of times where the null hypothesis is rejected, is expected to show an upward trend, and eventually close to $1$. 


```{r p3 analysis and plotting - estimate mu vs. estimate mu reject}
# Average Estimate of μ^ vs. True μ
estimate_results = sim_results |>
  mutate(reject = estimate_mu * reject_null) |>
  group_by(true_mu) |>
  summarize(
    avg_estimate_mu = mean(estimate_mu),
    avg_estimate_mu_reject = mean(reject)
  ) |>
  pivot_longer(
...
The average estimate of $\hat{\mu}$ tends to approach the true value of $\mu$, as $\mu$ increases. This occurs because as the true $\mu$ moves further from zero, the likelihood of rejecting the null hypothesis increases. Consequently, the rejected group's $\hat{\mu}$ values move closer to the true $\mu$. Therefore, the average estimate $\hat{\mu}$ for the rejected cases should converge to the overall average estimate of $\hat{\mu}$.
